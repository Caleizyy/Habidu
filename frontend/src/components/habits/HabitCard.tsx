...
export default function HabitCard({label, choices} : choicesProps ) {
    ...
    const habitSelectChoices = habitSelectOptions.map(option => 
        <Select key={option.label}>
                        ...
                        <SelectContent>
                            <SelectGroup>
                                {
                                option.choices.map(choice => <SelectItem key={choice} value= {choice}> {choice}</SelectItem>
                                )
                            }
                            </SelectGroup>
                        </SelectContent>
        ...
        <Card className="w-93 h-130">
            ...
            <Dialog>
                ...
                <DialogContent>
                    ...
                    <Input placeholder = "Enter habit name" className="mb-8"/> {/* This text area is used for entering the habit name */}
                    {habitSelectChoices}
